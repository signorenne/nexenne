...
- Scout for reuse before writing new code. Check whether an existing component
  ...
  larger ones over duplicating logic. See [[file:DESIGN.org][DESIGN.org]].
...
- Every public symbol has Javadoc-flavored Doxygen: =/** ... */=, no HTML and no
  backticks (use =\c= for inline code). Keep the tag order from
  [[file:STYLE_GUIDE.org][STYLE_GUIDE.org]], including =@pre= and =@post=, and
